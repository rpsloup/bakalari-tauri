...
const Timetable = ({ timetable }: IProps): JSX.Element => {
  const minHourId = Math.min(
    ...[
      ...timetable.Days.map((day) => day.Atoms.map((atom) => atom.HourId)),
    ].flat()
  );
  const maxHourId = Math.max(
    ...[
      ...timetable.Days.map((day) => day.Atoms.map((atom) => atom.HourId)),
    ].flat()
  );
  ...
  return (
    <table className="timetable">
      <tr className="hour-row">
        <td>&nbsp;</td>
        {timetable.Hours.filter(
          (hour) => hour.Id >= minHourId && hour.Id <= maxHourId
        ).map((hour, hourIndex) => (
